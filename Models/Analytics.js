...
class Analytics {
    ...
    static async AlltimepopulateAnalyticsFromLogs() {
        try {
            // Fetch all call records from the CDR log table
            const logs = await DataBase_1.default.getAll(Tabels_1.CDR_LOGS_TABLE, "*", "", "");
            if (!logs || logs.length === 0)
                return;
            // Aggregate data by accountcode and date
            const analyticsData = {};
            logs.forEach((log) => {
                const date = log.start.split("T")[0]; // Extract date (YYYY-MM-DD)
                const key = `${log.accountcode}_${date}`;
                if (!analyticsData[key]) {
                    analyticsData[key] = {
                        id: Snowflake_1.default.nextId(),
                ...
                // Track busiest hour
                const hour = log.start.split("T")[1].split(":")[1]; // Extract hour (HH)
            ...
            for (const key in analyticsData) {
                ...
                const existingRecord = await DataBase_1.default.getAll(Tabels_1.CDR_ANALYTICS_TABLE, "*", "", "WHERE accountcode = ? AND date = ?", [
                    entry.accountcode,
                    entry.date,
                ]);
                if (existingRecord.length > 0) {
                    // Update existing record
                    await DataBase_1.default.update(existingRecord[0].id, {
                    ...
                    }, Tabels_1.CDR_ANALYTICS_TABLE);
                    await this.saveAlltimeData({
                        id: Alltimedata + entry.accountcode,
                        accountcode: entry.accountcode,
                        date: entry.date,
                        total_calls: entry.total_calls - existingRecord[0].total_calls,
                        answered_calls: entry.answered_calls - existingRecord[0].answered_calls,
                        missed_calls: entry.missed_calls - existingRecord[0].missed_calls,
                        failed_calls: entry.failed_calls - existingRecord[0].failed_calls,
                        total_duration: entry.total_duration - existingRecord[0].total_duration,
                        avg_duration: entry.avg_duration,
                        busiest_hour: entry.busiest_hour,
                        most_called_number: entry.most_called_number,
                        most_frequent_caller: entry.most_frequent_caller,
